...
grounded in a finished run's `result` (and `state`, as a fallback for detail
beyond what's in the fixed output contract), not a stage that can mutate the
run. No actions/tool-calling in v1 — every reply is a single call_llm() text
call, no response_model, no task_queue (see routers/agents.py chat route
...
class AgentChatError(Exception):
    """Raised for chat-specific failure conditions (run not found / not owned / not completed)."""
...
def _format_run_context(run: dict) -> str:
    ...
    return (
        ...
        f"Extra detail (e.g. evaluation plan, full per-criterion scores, full ranking): "
        f"{result.get('extra', {})}\n\n"
        f"Full internal run state (fallback if the above doesn't have the answer): {state}"
    )
...
def handle_chat_message(run_id: str, message: str, user_id: str) -> dict:
    ...
    user_id, or isn't completed yet — callers should map this to a 400.
    """
