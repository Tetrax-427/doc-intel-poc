...
# ---------------------------------------------------------------------------
# Field extraction — flat schemas (templates, custom flat field dicts)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Field extraction — nested/dynamic schemas (lists of objects, nested objects)
# ---------------------------------------------------------------------------
#
# Used by retrieval.extract_dynamic_fields() whenever the request supplies a
# SchemaSpec (schemas.dynamic) instead of a flat {name: description} dict —
# i.e. inline `nested_schema` on POST /extract, or any schema generated by
# schemas.dynamic.generate_schema_spec() via POST /extract/nl.
#
# user side assembled inline in retrieval.extract_dynamic_fields():
# f"{correction_examples}Fields to extract:\n{field_desc}\n\n{sandboxed_context}"

EXTRACTION_SYSTEM_NESTED = (
    "You are a precise data extraction assistant. "
    "Extract the requested fields — including nested objects and repeating lists — "
    "from the document below.\n\n"
    "Rules:\n"
    "- Extract ONLY what is explicitly stated in the document\n"
    "- For list fields, return one entry per occurrence found in the document — "
    ...
    "totals, sums, or growth rates yourself — extract only what is directly stated, "
    "even if a field name suggests a computation\n"
    "- Do NOT mix up entities across list items (e.g. don't attribute one company's dates "
    "to a different company in the same list)\n"
    "- Return ONLY a valid JSON object, no explanation, no markdown fences"
)


# ---------------------------------------------------------------------------
# Schema generation — free-text description -> SchemaSpec
# ---------------------------------------------------------------------------
#
# Used by schemas.dynamic.generate_schema_spec(), called from:
#   - retrieval.extract_nl() for every POST /extract/nl request
#   - routers/extraction.py preview_only path
#
# The response is coerced into schemas.dynamic.SchemaSpec by Instructor —
# no JSON format instruction needed here, same pattern as
# DOCUMENT_CLASSIFIER_SYSTEM below.
#
# user side: f"Description: {description}"

SCHEMA_GEN_SYSTEM = (
    "You are a schema-design assistant. Given a user's description of what they want "
    "extracted from a document — for any domain (finance, HR, legal, operations, etc.) — "
    "produce a structured field specification.\n\n"
    "Rules:\n"
    "- Use type='list' with `properties` set to the repeating item's fields for anything "
    "that repeats (line items, quarterly entries, employment history, etc.)\n"
    "- Use type='object' with `properties` for a single nested sub-structure\n"
    "- Use a primitive type ('string', 'number', 'integer', 'date', 'boolean') for "
    "everything else — do not set `properties` on primitive fields\n"
    "- Field names must be valid snake_case identifiers\n"
    "- Do NOT invent fields the user did not imply\n"
    "- Do NOT include computed or derived fields (totals, durations, growth rates, "
    "averages, etc.) — those are computed after extraction, never by you\n"
    "- Keep field descriptions short and specific — they directly guide what gets "
